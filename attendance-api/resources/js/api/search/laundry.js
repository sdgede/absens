...
searchInput.addEventListener("input", function (e) {
    initSearchHandler({
        ...
        apiPath: "/admin/laundry",
        renderFn: renderLaundryList,
    });
});

function renderLaundryList(data) {
    ...
    if (data.data.length === 0) {
        laundryList.innerHTML = `
            <tr>
                <td class="px-6 py-4 text-sm text-primary text-center" colspan="7">No laundry data</td>
            </tr>
        `;
    ...
    data.data.forEach((item, index) => {
        laundryList.innerHTML += `
            <tr>
                ...
                <td class="px-6 py-4 text-sm text-primary">${item?.name_laundry}</td>
                ...
                <td class="px-6 py-4 flex items-center gap-2 text-sm text-gray-500">
                    ...
                    <button 
                        data-modal-target="modalEditLaundry"
                        data-id="${item?.id}"
                        data-modal-key="showModalEditLaundry"
