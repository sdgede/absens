...
searchInput.addEventListener("input", function (e) {
    initSearchHandler({
        ...
        apiPath: "/admin/canceled",
        renderFn: renderCanceledList,
    });
});

function renderCanceledList(data) {
    ...
    data.data.forEach((item, index) => {
        canceledList.innerHTML += `
            <tr>
                ...
                <td class="px-6 py-4 text-sm text-primary">${item?.issues}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <button 
                        data-modal-target="modalInformationCanceled"
                        data-id="${item?.id}"
